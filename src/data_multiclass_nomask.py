...
class StreamingHFDataset(IterableDataset):
    ...
    def __iter__(self):
        for example in self.dataset:
            ...
            # Optional: decode mask the same way if it’s also stored as dict
...
def mask_unaware_crop(img, mask=None, crop_size=224, crop_big=518): #was 512 and not 518 that is only for DINOv2
    ...
    if mask is None:
        mask_tensor = torch.zeros((crop_big, crop_big), dtype=torch.uint8) #was crop_size previously
    else:
        # Ensure mask is PIL for these transforms
