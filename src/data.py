...
class EvaluationDataset(Dataset):
    def __init__(self, generator, transforms=None, perturb=None):
        df = pd.read_csv("/home/mbrigo/RINE/texturecrop_datasets/"+generator+".csv") #ADDED
        valid_images = set(df['filepath'].tolist()) #ADDED
        ...
        self.real = [(path, label) for path, label in self.real if os.path.basename(path) in valid_images] #ADDED
        self.fake = [(path, label) for path, label in self.fake if os.path.basename(path) in valid_images] #ADDED
        self.images = self.real + self.fake
